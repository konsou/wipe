...
def main():
    ...
    logdir = os.path.join(user_home_dir, "wipe-logs")
    ...
    system_devices = drive.system_devices()
    non_system_devices = drive.get_devices(exclude_system_devices=True)

    log("/dev/stdout", "/dev/stdout", f"System Devices: {system_devices}")

    for device in non_system_devices:
